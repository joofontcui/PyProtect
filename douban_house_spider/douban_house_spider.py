...
f = open(r'demo3.txt', "w")

j = 0
for j in range(0,250,25):
    url = "https://www.douban.com/group/zhufang/discussion?start=%d" % j
    ...
    for i in inHtml:
        ...
        z = h+j
        print z
        f.write('------------最近第'+ str(z)+'条：'+inTitle.encode('utf8')+'------------\n')  # 房屋标题
        onTime = soup.select('.color-green')
        for inT in onTime:
            for inTi in inT.strings:
                print inTi
                f.write('发布时间：' + inTi.encode('utf8') + '\n')
            # inTime = re.findall(r'<span class="color-green">(.+?)</span>', inT, re.I)

        # for inTime in onTime:
        #     f.write('发布时间：'+inTime.string.encode('utf8')+'\n')
        f.write('豆瓣原文链接：'+i.encode('utf8')+'\n')  # 豆瓣原链接
        ...
        try:
            for oonText in onText:
                ...
                f.write('详细描述：'+strNeed.encode('utf8')+'\n')
            h+=1
            print '------------------'
            f.write('\n\n')
        except:
